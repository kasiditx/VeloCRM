                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-1">
                                        @if ($showTrashed)
                                            ...
                                            <button wire:click="forceDelete({{ $lead->id }})" wire:loading.attr="disabled" wire:target="forceDelete({{ $lead->id }})" data-velo-confirm="{{ __('Permanently delete this lead?') }}" title="{{ __('Delete Permanently') }}" aria-label="{{ __('Delete Permanently') }}"
                                        ...
                                        @else
                                            ...
                                            <button wire:click="delete({{ $lead->id }})" wire:loading.attr="disabled" wire:target="delete({{ $lead->id }})" data-velo-confirm="{{ __('Move this lead to trash?') }}" title="{{ __('Delete') }}" aria-label="{{ __('Delete') }}"
                                                class="module-icon-button hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                            </button>
                                        @endif
                    ...
                    <div class="module-mobile-card">
                        ...
                        <div class="flex gap-2 pt-2 border-t border-gray-100 dark:border-gray-700/50">
                            @if ($showTrashed)
                                ...
                                <button wire:click="forceDelete({{ $lead->id }})" wire:loading.attr="disabled" wire:target="forceDelete({{ $lead->id }})" data-velo-confirm="{{ __('Permanently delete this lead?') }}" class="flex-1 rounded-lg bg-rose-50 py-2 text-xs font-semibold text-rose-600 disabled:pointer-events-none disabled:opacity-50 dark:bg-rose-900/30 dark:text-rose-400"><x-ui.loading-label target="forceDelete({{ $lead->id }})" :label="__('Delete')" :loading="__('Deleting...')" /></button>
                            @else
